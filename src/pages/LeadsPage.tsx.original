...
import { enviarLeadParaSheets, atualizarEtapaNaSheets } from '@/hooks/useGoogleSheetsSync';
...
export default function LeadsPage() {
      ...
      <div className="p-6 text-center">
        ...
        <Dialog open={newPipelineDialog} onOpenChange={setNewPipelineDialog}>
          <DialogContent>
            ...
            <Input placeholder="Nome do funil" value={newPipelineName} onChange={(e) => setNewPipelineName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleCreatePipeline()} />
            <DialogFooter>
  ...
  const items = pipelineLeads.map((l) => ({
    id: l.id, nome: l.nome, telefone: l.telefone, colunaId: l.colunaId,
    criadoEm: l.criadoEm, origem: l.origem, tarefas: l.tarefas,
  }));
  ...
  const handleAddLead = () => {
    ...
    crm.addLead({
      ...
      origem: form.origem, colunaId: addDialog, pipelineId,
    });
    // Send to Google Sheets
    enviarLeadParaSheets(form.nome, form.telefone, form.origem);
  ...
  const handleConvert = (id: string) => {
    ...
    if (firstCol) {
      ...
      crm.convertLeadToPaciente(id, firstCol.id);
      setSelectedLeadId(null);
      // Atualiza etapa na planilha para "Paciente"
      if (lead) atualizarEtapaNaSheets(lead.telefone, 'Paciente');
    }
  };
  ...
  return (
    <div className="p-6">
      ...
      <KanbanBoard
        ...
        items={items}
        onItemMove={(id, colId) => {
          crm.updateLead(id, { colunaId: colId });
          // Atualiza etapa na planilha com o nome da coluna
          const lead = crm.leads.find(l => l.id === id);
          const coluna = pipeline?.colunas.find(c => c.id === colId);
          if (lead && coluna) {
            atualizarEtapaNaSheets(lead.telefone, coluna.nome);
          }
        }}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tem certeza que deseja excluir "{pipeline.nome}"? Todos os leads deste funil serão removidos.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
